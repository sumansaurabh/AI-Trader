...
- [TwinMarket](https://github.com/FreedomIntelligence/TwinMarket): Un marco multi-agente que aprovecha LLMs para simular el comportamiento de inversores y fenómenos socioeconómicos emergentes en el mercado de acciones A-share.
---
...
- ✅ **Soporte de Trading por Hora** - Hemos actualizado de intervalos de trading diarios a horarios, permitiendo una participación en el mercado más precisa y receptiva con control de tiempo granular.
...
- ✅ **Visualización de Razonamiento del Agente** - Implementamos transparencia completa en los procesos de toma de decisiones de IA, con cadenas de razonamiento detalladas que muestran cómo se forma cada decisión de trading.
...
[🚀 Inicio Rápido](#-inicio-rápido) • [📈 Análisis de Rendimiento](#-análisis-de-rendimiento) • [🛠️ Guía de Configuración](#-guía-de-configuración) • [中文文档](README_CN.md) • [English](README.md)
...
- 🔍 **Inteligencia de Mercado Inteligente**: Búsqueda Jina integrada para noticias de mercado en tiempo real e informes financieros
...
- ⏰ **Horario de Trading**: Toda la semana para criptomonedas, horario de mercado en días laborables para acciones con soporte de simulación histórica
...
- 🔄 **Evolución de Estrategia Adaptativa**: Algoritmos auto-optimizantes que se ajustan basándose en retroalimentación del rendimiento del mercado
...
🎯 **Objetivo**: ¡Determinar qué modelo de IA logra retornos de inversión superiores a través de operación puramente autónoma!

### 🚫 Cero Intervención Humana
Los agentes de IA operan con completa autonomía, tomando todas las decisiones de trading y ajustes de estrategia sin ninguna programación humana, orientación o intervención.
...
- 📊 **Límites de Datos de Precios**: Acceso a datos de mercado limitado a marca de tiempo de simulación y registros históricos
- 📰 **Aplicación de Cronología de Noticias**: Filtrado en tiempo real previene acceso a noticias y anuncios con fechas futuras
...
- 🧠 **Análisis de Consistencia de Decisiones**: Examinar estabilidad temporal y patrones de comportamiento en la lógica de trading de IA
...
AI-Trader Bench/
├── 🤖 Sistema Central
│   ├── main.py                    # 🎯 Entrada del programa principal
│   ├── agent/
...
│   │   ├── tool_get_price_local.py # 📊 Consultas de precios (soporta acciones estadounidenses + A-shares)
...
└── 🚀 Scripts de Inicio Rápido
    └── scripts/                   # 🛠️ Scripts de inicio convenientes
        ...
        ├── main_crypto_step1.sh   # Criptomonedas: Preparación de datos
        ├── main_crypto_step2.sh   # Criptomonedas: Iniciar servicios MCP
        ├── main_crypto_step3.sh   # Criptomonedas: Ejecutar agente de trading
...
- **Carga Dinámica de Agentes**: Cargar automáticamente el tipo de agente correspondiente basado en la configuración
...
| Tipo de Agente | Ruta del Módulo | Caso de Uso | Características |\n|-----------|-------------|----------|----------|\n| **BaseAgent** | `agent.base_agent` | Genérico para acciones estadounidenses/A-shares | Cambio de mercado flexible, pool de acciones configurable |\n| **BaseAgentAStock** | `agent.base_agent_astock` | Específico para A-share | Reglas A-share integradas, pool predeterminado SSE 50, prompts en chino |\n| **BaseAgentCrypto** | `agent.base_agent_crypto` | Específico para criptomonedas | Pool de criptomonedas BITWISE10, denominado en USDT |

**Ventajas de la Arquitectura**:
- 🔄 **Separación Clara**: Agentes de acciones estadounidenses, A-share y criptomonedas mantenidos independientemente sin interferencia
- 🎯 **Optimización Especializada**: Cada agente profundamente optimizado para características específicas del mercado
- 🔌 **Fácil Extensión**: Soporte para agregar más agentes específicos de mercado (por ejemplo, acciones de Hong Kong, commodities)

#### 🛠️ Cadena de Herramientas MCP
| Herramienta | Función | Soporte de Mercado | API |
|------|----------|----------------|-----|
| **Herramienta de Trading** | Comprar/vender activos, gestión de posiciones | 🇺🇸 EE.UU. / 🇨🇳 A-shares / ₿ Criptomonedas | `buy()`, `sell()` / `buy_crypto()`, `sell_crypto()` (Para Criptomonedas)|
| **Herramienta de Precios** | Consultas de precios en tiempo real e históricos | 🇺🇸 EE.UU. / 🇨🇳 A-shares / ₿ Criptomonedas | `get_price_local()` |
...
- 🔍 **Auto-Reconocimiento**: Seleccionar automáticamente la fuente de datos basada en el formato del símbolo (códigos de acciones o símbolos de criptomonedas)
- 📏 **Adaptación de Reglas**: Auto-aplicar reglas de trading del mercado correspondiente (T+0/T+1, tamaños de lote, etc.)
...
- **📝 Registros de Trading**:
  ...
  - Almacenado por separado por mercado: `agent_data/` (EE.UU.), `agent_data_astock/` (A-shares), `agent_data_crypto/` (Criptomonedas)
...
ALPHADVANTAGE_API_KEY=your_alpha_vantage_key  # Para datos NASDAQ 100
...
```bash
# Ejecutar paso a paso:
bash scripts/main_crypto_step1.sh  # Paso 1: Preparar datos de criptomonedas
bash scripts/main_crypto_step2.sh  # Paso 2: Iniciar servicios MCP
bash scripts/main_crypto_step3.sh  # Paso 3: Ejecutar agente de trading de criptomonedas
```
...
```json
{
  ...
  "market": "cn",                   // Tipo de mercado: "cn" A-shares (opcional, será ignorado, siempre usa cn)
  "date_range": {
...
| **🌍 Mercado** | Mercado de Acciones Estadounidense | Mercado A-Share de China | Mercado Global de Criptomonedas |
| **⏰ Horario de Trading** | Días laborables | Días laborables | Toda la Semana |
...
| `initial_cash` | Capital inicial | Flotante | $10,000 (EE.UU.)<br>¥100,000 (A-shares) <br> 50,000-USDT (Criptomonedas) |
...
| **BaseAgentCrypto** | Específico Criptomonedas | • BITWISE 10 predeterminado<br>• Precios en USDT |
...
**🛠️ Ejecución pura basada en herramientas con cero intervención humana—¡una genuina arena de trading de IA!** 🚀
