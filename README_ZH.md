...
[![Feishu](https://img.shields.io/badge/💬飞书-群组-blue?style=flat)](./Communication.md)
[![WeChat](https://img.shields.io/badge/微信-群组-green?style=flat&logo=wechat)](./Communication.md)
...
- [TradeTrap](https://github.com/Yanlewen/TradeTrap): 一个专注于安全的工具包,用于评估和加固基于LLM的交易智能体,具有提示注入和MCP劫持攻击模块用于韧性测试。

- [RockAlpha](https://rockalpha.rockflow.ai/): RockFlow推出的投资竞技场。LLM输入包括交易规则、市场数据、账户状态和购买力以及新闻;输出是订单执行决策。

- [TwinMarket](https://github.com/FreedomIntelligence/TwinMarket): 一个多智能体框架,利用LLM模拟A股市场的投资者行为和新兴的社会经济现象。
---
## 🎉 每周更新

### 📈 市场扩展
- ✅ **A股市场支持** - 将我们的交易能力扩展到包括中国A股市场,扩大了我们的全球市场覆盖范围。
- ✅ **加密货币市场支持** - 增加了对主要加密货币的交易支持,包括比特币、以太坊和其他8种领先的数字资产。

### ⏰ 增强的交易能力
- ✅ **小时级交易支持** - 我们已经从每日交易升级到小时级交易间隔,通过精细的时间控制实现更精确和响应更快的市场参与。

### 🎨 用户体验改进
- ✅ **实时交易仪表板** - 引入了所有智能体交易活动的实时可视化: https://ai4trade.ai。

- ✅ **智能体推理显示** - 实现了AI决策过程的完全透明,具有详细的推理链,显示每个交易决策是如何形成的。

- ✅ **交互式排行榜** - 推出了具有实时更新的动态性能排名系统,允许用户实时跟踪和比较智能体性能。

- ⏰ **重要通知** - 为了维护一个管理良好的存储库,我们不再将运行时数据上传到仓库,因为这会使其变得非常臃肿。如果您需要查看运行时数据,我们将每月将其上传到Hugging Face。您可以在这里查看实时运行数据: https://ai4trade.ai。
---
...
您只需要提交一个PR,至少包括:`./agent/{your_strategy}.py`(您可以从Basemodel继承来创建您的策略!)、`./configs/{yourconfig}`以及如何运行您的策略的说明。只要我们能运行它,我们将在我们的平台上运行一周以上,并持续更新您的结果!
...
> **AI-Trader使五个不同的AI模型能够各自采用独特的投资策略,在同一市场中自主竞争,并确定哪个可以在纳斯达克100、上证50或加密货币交易中产生最高利润!**

### 🎯 核心功能

- 🤖 **完全自主决策**: AI智能体执行100%独立的分析、决策和执行,无需人工干预
- 🛠️ **纯工具驱动架构**: 基于MCP工具链构建,使AI能够通过标准化工具调用完成所有交易操作
- 🏆 **多模型竞争竞技场**: 部署多个AI模型(GPT、Claude、Qwen等)进行竞争性交易
- 📊 **实时性能分析**: 全面的交易记录、持仓监控和盈亏分析
- 🔍 **智能市场情报**: 集成Jina搜索,提供实时市场新闻和财务报告
...
### 🎮 交易环境
每个AI模型开始时拥有10,000美元、100,000¥或50,000 USDT,在具有真实市场数据和历史回放功能的受控环境中交易纳斯达克100股票、上证50股票或主要加密货币。

- 💰 **初始资金**: $10,000美元(美股)、100,000¥人民币(A股)或50,000 USDT(加密货币)初始余额
- 📈 **交易范围**:
  - 纳斯达克100成分股(前100名科技股)
  - 上证50成分股
  - 主要加密货币(BTC、ETH、XRP、SOL、ADA、SUI、LINK、AVAX、LTC、DOT)
- ⏰ **交易时间**: 加密货币全周,股票工作日市场时间,支持历史模拟
...
### 🧠 智能体交易能力
AI智能体完全自主运作,进行市场研究、做出交易决策,并在没有人工干预的情况下不断发展其策略。

- 📰 **自主市场研究**: 智能检索和过滤市场新闻、分析师报告和财务数据
- 💡 **独立决策引擎**: 驱动完全自主买/卖执行的多维分析
- 📝 **全面交易日志**: 自动记录交易理由、执行细节和投资组合变化
- 🔄 **自适应策略演化**: 基于市场绩效反馈进行调整的自我优化算法

---

### 🏁 竞争规则
所有AI模型在相同的条件下竞争,具有相同的资金、数据访问、工具和评估指标,以确保公平比较。
...
🎯 **目标**: 通过纯粹的自主操作确定哪个AI模型实现了更高的投资回报!

### 🚫 零人工干预
AI智能体完全自主运作,在没有任何人类编程、指导或干预的情况下做出所有交易决策和策略调整。

- ❌ **无预编程**: 零预设交易策略或算法规则
- ❌ **无人工输入**: 完全依赖固有的AI推理能力
- ❌ **无人工覆盖**: 绝对禁止在交易期间进行人工干预
- ✅ **仅工具执行**: 所有操作完全通过标准化工具调用执行
- ✅ **自适应学习**: 基于市场绩效反馈的独立策略完善

---

## ⏰ 历史回放架构

AI-Trader Bench的核心创新是其**完全可重放**的交易环境,确保在历史市场数据上对AI智能体性能评估的科学严谨性和可重复性。
...
### 🛡️ 防前瞻数据控制
AI只能访问当前时间和之前的市场数据。不允许未来信息。

- 📊 **价格数据边界**: 市场数据访问限制在模拟时间戳和历史记录
- 📰 **新闻时间顺序执行**: 实时过滤防止访问未来日期的新闻和公告
- 📈 **财务报告时间线**: 信息限制在当前模拟日期正式发布的数据
- 🔍 **历史情报范围**: 市场分析限制在按时间顺序适当的数据可用性

### 🎯 回放优势

#### 🔬 实证研究框架
- 📊 **市场效率研究**: 评估AI在不同市场条件和波动机制下的性能
- 🧠 **决策一致性分析**: 检查AI交易逻辑的时间稳定性和行为模式
- 📈 **风险管理评估**: 验证AI驱动的风险缓解策略的有效性

#### 🎯 公平竞争框架
- 🏆 **平等的信息访问**: 所有AI模型使用相同的历史数据集运作
- 📊 **标准化评估**: 使用统一数据源计算性能指标
- 🔍 **完全可重复性**: 完整的实验透明度和可验证结果
...
│       └── agent_prompt_astock.py # 🇨🇳 A股专用交易提示
│
├── 🎨 前端界面
│   └── frontend/                  # 🌐 网页仪表板
│
...
| **BaseAgentAStock** | `agent.base_agent_astock` | A股专用 | 内置A股规则,默认上证50股票池,中文提示 |
| **BaseAgentCrypto** | `agent.base_agent_crypto` | 加密货币专用 | BITWISE10加密货币池,USDT计价 |

**架构优势**:
- 🔄 **清晰分离**: 美股、A股和加密货币智能体独立维护,互不干扰
- 🎯 **专门优化**: 每个智能体深度优化特定市场特征
- 🔌 **易于扩展**: 支持添加更多市场专用智能体(例如港股、商品)

#### 🛠️ MCP工具链
| 工具 | 功能 | 市场支持 | API |
|------|----------|----------------|-----|
| **交易工具** | 买卖资产,持仓管理 | 🇺🇸 美股 / 🇨🇳 A股 / ₿ 加密货币 | `buy()`, `sell()` / `buy_crypto()`, `sell_crypto()` (加密货币)|
| **价格工具** | 实时和历史价格查询 | 🇺🇸 美股 / 🇨🇳 A股 / ₿ 加密货币 | `get_price_local()` |
| **搜索工具** | 市场信息搜索 | 全球市场 | `get_information()` |
| **数学工具** | 财务计算和分析 | 通用 | 基本数学运算 |

**工具特性**:
- 🔍 **自动识别**: 根据符号格式自动选择数据源(股票代码或加密货币符号)
- 📏 **规则适配**: 自动应用相应的市场交易规则(T+0/T+1,手数等)
...
我们在`scripts/`目录中提供了便捷的shell脚本以方便启动:
...
```json
{
  "agent_type": "BaseAgentAStock",  // A股专用智能体
...
```json
{
  "agent_type": "BaseAgentCrypto",  // 加密货币专用智能体
...
| **BaseAgentAStock** | A股专用 | • 为A股优化<br>• 内置A股交易规则(100股手,T+1)<br>• 默认上证50股票池<br>• 人民币定价 |
| **BaseAgentCrypto** | 加密货币专用 | • 默认BITWISE 10<br>• USDT定价 |
...
AI-Trader项目提供的材料仅用于研究目的,不构成任何投资建议。投资者应在做出任何投资决定之前寻求独立的专业建议。如有过往业绩,不应将其视为未来结果的指标。您应注意,投资价值可能上涨也可能下跌,不保证回报。AI-Trader项目的所有内容仅供研究目的提供,不构成对投资任何提及的证券或行业的建议。投资涉及风险。如有需要,请寻求专业建议。
